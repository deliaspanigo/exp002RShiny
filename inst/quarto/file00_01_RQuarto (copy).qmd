...
# # # # # Section 02 - Import dataset ----------------------------------------
  #+++---my_dataset <- _+A+_import_dataset_internal_+A+_
  head(x = my_dataset, n = 5)
```

```{r eval=TRUE, echo=TRUE}
# # # # # Section 02 - Import dataset ----------------------------------------
  #+++---my_dataset <- _+A+_import_dataset_external_+A+_
  head(x = my_dataset, n = 5)
```
...
# # # # # Section 06 - Selected variables and roles  -------------------------
  ...
  minibase[,var_name_factor] <- factor(
  x = minibase[,var_name_factor],       # La variable original de factor
  ...
  df_table_anova_mod <- df_table_anova
  new_row_anova <- nrow(df_table_anova_mod)+ 1
  df_table_anova_mod[new_row_anova, ] <- rep(NA, ncol(df_table_anova_mod))
  df_table_anova_mod$"Df"[new_row_anova] <- sum(na.omit(df_table_anova_mod$"Df"))
  df_table_anova_mod$"Sum Sq"[new_row_anova] <- sum(na.omit(df_table_anova_mod$"Sum Sq"))
  df_table_anova_mod$"Mean Sq"[new_row_anova] <- df_table_anova_mod$"Sum Sq"[new_row_anova]/df_table_anova_mod$"Df"[new_row_anova]
  df_table_anova_mod <- cbind.data.frame(c("Factor", "Error", "Total"), df_table_anova_mod)
  colnames(df_table_anova_mod)[1]  <- "Sources of Variation"
  rownames(df_table_anova_mod)[new_row_anova]  <- "Total"
  df_table_anova_mod
  ...
  df_tukey_table <- data.frame(
   ...
   df_tukey_table[,"level"] <- factor(
  x = df_tukey_table[,"level"],       # La variable original de factor
  levels = df_tukey_table[,"level"]  # El orden de los niveles que calculamos en el Paso 2
)
  df_tukey_table
  ...
  df_vr_position_levels[,"level"] <- factor(
  x = df_vr_position_levels[,"level"],       # La variable original de factor
  levels = df_vr_position_levels[,"level"]  # El orden de los niveles que calculamos en el Paso 2
)
  ...
  df_vr_dispersion_levels <- data.frame(
   ...
   df_vr_dispersion_levels[,"level"] <- factor(
  x = df_vr_dispersion_levels[,"level"],       # La variable original de factor
  levels = df_vr_dispersion_levels[,"level"]  # El orden de los niveles que calculamos en el Paso 2
)
...
df_table_factor_plot002[,"level"] <- factor(
  ...
  for (k in 1:n_levels){
    ...
    plot006_factor <- plot006_factor %>%
      add_trace(y = minibase_mod[,2][dt_filas],
                x = minibase_mod[,1][dt_filas],
                type = "violin",
                orientation = 'h',
                name = paste0("violin", k),
                side = "positive",
